/*
* This program determines whether a given string is palindrome or not. Each 
* character is considered to be a node and the whole string is a singly linked 
* list. The comparison is done sequentially after reversing the string and 
...
while (s1 != NULL) {
	if (s1 -> ch < s2 -> ch) //return -1 if s1 < s2,
		return -1;
	else if (s1 -> ch > s2 -> ch)
		return 1;
	s1 = s1 -> next;
